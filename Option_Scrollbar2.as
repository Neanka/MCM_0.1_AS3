package {
    ...
    public class Option_Scrollbar1 extends MovieClip {
		...
		public static const MOUSE_OVER_THUMB:String = "Option_Scrollbar1::MOUSE_OVER_THUMB";
		public static const MOUSE_OUT_THUMB:String = "Option_Scrollbar1::MOUSE_OUT_THUMB";
		...
		public var bIsDragging:Boolean = false;
        private var fStartValue:Number;
		public var oldvalue: Number;

        public function Option_Scrollbar1(naMax: Number,naMin:Number,naStep:Number){
			fMinValue = naMin;
			fMaxValue = naMax;
			fStepSize = naStep;
			super();
			Track_mc = new MovieClip();
			Track_mc.graphics.beginFill(0xCCCCCC,0.3);
			Track_mc.graphics.drawRect(0,0,340,20);
			Track_mc.graphics.endFill();
			addChild(Track_mc);
			Thumb_mc = new MovieClip();
			Thumb_mc.graphics.beginFill(0xFFFFFF,1);
         ...
         //   this.Thumb_mc.addEventListener(MouseEvent.MOUSE_DOWN, this.onThumbMouseDown);
			...
			//stage.addEventListener(Main.MOUSE_DOWN, this.onThumbMouseDown);
        }
        ...
        private function onThumbMouseOver(event:MouseEvent){
			dispatchEvent(new Event(MOUSE_OVER_THUMB, true, true));
        }
        private function onThumbMouseOut(event:MouseEvent){
			dispatchEvent(new Event(MOUSE_OUT_THUMB, true, true));
        }
		public function startdraggingfromevent():*{
			bIsDragging = true;
			this.Thumb_mc.startDrag(false, new Rectangle(0, this.Thumb_mc.y, (this.fMaxThumbX - this.fMinThumbX), 0));
			stage.addEventListener(MouseEvent.MOUSE_MOVE, this.onThumbMouseMove);
		}
		public function stopdraggingfromevent():*{
            this.Thumb_mc.stopDrag();
            stage.removeEventListener(MouseEvent.MOUSE_MOVE, this.onThumbMouseMove);
		}
    }
